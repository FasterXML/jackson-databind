        ...
        java.io.Serializable // as of 2.1
{
    ...
    */
    ...
    public List<Object> readValue(String content, List<JavaType> elemenTypeList)
            throws JsonProcessingException, JsonMappingException {
        _assertNotNull("content", content);
        try { // since 2.10 remove "impossible" IOException as per [databind#1675]
            return _readMapAndClose(_jsonFactory.createParser(content), elemenTypeList);
        } catch (JsonProcessingException e) {
            throw e;
        } catch (IOException e) { // shouldn't really happen but being declared need to
            throw JsonMappingException.fromUnexpectedIOE(e);
    ...
    @SuppressWarnings("unchecked")
    protected List<Object> _readMapAndClose(JsonParser p0, List<JavaType> elemenTypeList)
        throws IOException
    {
        try (JsonParser p = p0) {
