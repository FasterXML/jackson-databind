            ...
            Annotated ann, JavaType baseType)
    {
        ...
        if (customResolverOb != null) {
            ...
            if (ann instanceof AnnotatedClass annotatedClass) {
                ...
                // NOTE: if the annotation appears on multiple places, first instance
                // is matched; `AnnotatedClass.getSuperTypes()` documents the order AND
                // it is the order in which `JacksonAnnotationIntrospector` looks for
                // `@JsonTypeInfo` as well.
